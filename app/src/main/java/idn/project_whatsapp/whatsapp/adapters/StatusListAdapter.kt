...
import idn.project_whatsapp.whatsapp.listener.StatusItemClickListener
...
class StatusListAdapter(val statusList: ArrayList<StatusListElement>) :
    ...
    override fun onBindViewHolder(holder: StatusListViewHolder, position: Int) { holder.bindItem(statusList[position], clickListener)
    }
    ...
    class StatusListViewHolder(override val containerView: View) : RecyclerView.ViewHolder(containerView),
        LayoutContainer {
        fun bindItem(statusElement: StatusListElement, listener: StatusItemClickListener?) { populateImage(
            img_status_photo.context, statusElement.userUrl, img_status_photo, R.drawable. ic_user
        )
